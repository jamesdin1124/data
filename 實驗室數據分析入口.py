    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            if do_filter:
                ...
                filtered_df = df
                st.write(f"使用全部數據，行數：{len(filtered_df)}")

            group_column = st.selectbox("選擇用分組的列", filtered_df.columns)
            
            # 判斷分組列是否為數值型
            is_numeric_group = pd.api.types.is_numeric_dtype(filtered_df[group_column])
            
            if is_numeric_group:
                group_method = st.selectbox("選擇分組方法", ["二分法", "自定義閾值"])
                if group_method == "二分法":
                    median_value = filtered_df[group_column].median()
                    st.write(f"{group_column} 的中位數為: {median_value}")
                    filtered_df['group'] = (filtered_df[group_column] > median_value).astype(int)
                    st.write("已將數據分為兩組：0（小於等於中位數）和 1（大於中位數）")
                else:
                    threshold = st.number_input(f"輸入 {group_column} 的閾值", value=float(filtered_df[group_column].mean()))
                    filtered_df['group'] = (filtered_df[group_column] > threshold).astype(int)
                    st.write(f"已將數據分為兩組：0（小於等於{threshold}）和 1（大於{threshold}）")
                group_column = 'group'
            
            continuous_vars = st.multiselect("選擇連續變量", filtered_df.columns)
            age_column = st.selectbox("選擇年齡列", filtered_df.columns)
            age_interval = st.number_input("選擇年齡間隔 (年)", min_value=1, value=5)

            if st.button("開始分析"):
                unique_groups = filtered_df[group_column].dropna().unique()
                if len(unique_groups) != 2:
                    st.error(f"分組列 '{group_column}' 應該包含兩個唯一值，但實際包含 {len(unique_groups)} 個值。請選擇另一個分組列或調整分組方法。")
                else:
                    if continuous_vars:
                        numeric_summary_df = analyze_numeric_data(filtered_df, group_column, continuous_vars)
                        if not numeric_summary_df.empty:
                            ...
                            if valid_columns:
                                for selected_column in valid_columns:
                                    ...
                                    fig = create_distribution_plot(filtered_df, group_column, selected_column, t_p, u_p)
                                    st.plotly_chart(fig)
                                    
                                    st.subheader(f"{selected_column} 隨年齡化的趨勢")
                                    trend_fig = create_boxplot_trend(filtered_df, group_column, selected_column, age_column, age_interval)
...
def chi_square_analysis():
    ...
    uploaded_file = st.file_uploader("請選擇CSV文件", type="csv")
    if uploaded_file is not None:
        df = load_data(uploaded_file)
        if df is not None:
            st.write("數據預覽：")
            st.write(df.head())
            st.write(f"原始數據行數：{len(df)}")
            st.write("數據類型：")
            st.write(df.dtypes)
            ...
            if do_filter:
                filter_column = st.selectbox("選擇用於篩選的���", df.columns)
                is_numeric_column = pd.api.types.is_numeric_dtype(df[filter_column])
                ...
                st.write(f"篩選後的數據行數：{len(df)}")
            else:
                st.write(f"使用全部數據，行數：{len(df)}")
...
def column_data_query():
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            selected_column = st.selectbox("選擇要查詢的列", df.columns)

            if st.button("開始查詢"):
                value_counts = df[selected_column].value_counts().reset_index()
                value_counts.columns = [selected_column, '數量']
                
                st.subheader(f"{selected_column} 列資料統計")
                st.write(f"共有 {len(value_counts)} 個不同的值")
                st.dataframe(value_counts)

                # 提供下載選項
                csv = value_counts.to_csv(index=False)
                st.download_button(
                    label="下載查詢結果 CSV",
                    data=csv,
                    file_name=f"{selected_column}_data_query.csv",
                    mime="text/csv",
                )

                # 繪製條形圖
                fig = go.Figure(data=[go.Bar(x=value_counts[selected_column], y=value_counts['數量'])])
                fig.update_layout(title=f'{selected_column} 數據分佈', xaxis_title=selected_column, yaxis_title='數量')
                st.plotly_chart(fig)

    else:
        st.info("請上傳一個CSV文件來開始查詢。")
...
def main():
    ...
    elif analysis_type == "列資料查詢":  # 新增的條件
        column_data_query()
